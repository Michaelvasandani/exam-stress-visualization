...
// Get base path for assets based on whether we're on GitHub Pages or local
function getBasePath() {
  const isGitHubPages = window.location.hostname.includes("github.io");
  return isGitHubPages ? "/exam-stress-visualization/" : "/";
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // First try to load the sample data (which is much smaller)
  d3.json(`${basePath}json/sample.json`)
    .then(function (sampleData) {
      console.log("Successfully loaded sample data for visualization");

      // Use the sample data for all three exams as fallback
      midterm1Data = sampleData;
      midterm2Data = sampleData;
      finalData = sampleData;

      // Initialize the app with sample data
      initializeApp();

      // Now try to load the full datasets, and update if successful
      loadFullDatasets();
    })
    .catch(function (error) {
      console.error("Error loading sample data:", error);
      alert(
        "Unable to load visualization data. Please check your internet connection and try again."
      );
    });
});

// Function to load the full datasets
function loadFullDatasets() {
  const basePath = getBasePath();

  // Try to load each dataset separately
  d3.json(`${basePath}json/Midterm_1.json`)
    .then(function (data) {
      console.log("Successfully loaded Midterm 1 data");
      midterm1Data = data;
    ...
    .catch(function (error) {
      console.warn("Using sample data for Midterm 1:", error);
    });

  d3.json(`${basePath}json/Midterm_2.json`)
    .then(function (data) {
      console.log("Successfully loaded Midterm 2 data");
      midterm2Data = data;
    ...
    .catch(function (error) {
      console.warn("Using sample data for Midterm 2:", error);
    });

  d3.json(`${basePath}json/Final.json`)
    .then(function (data) {
      console.log("Successfully loaded Final data");
      finalData = data;
...
// Function to update all visualizations based on the current page
function updateAllVisualizations() {
  if (currentPage === "body-map") {
    ...
    // Update any landing page visualizations that depend on the data
    resetMetricsGrid();
  }
}
...
function initializeApp() {
  ...
  // Populate student checkboxes if we're on a page that needs them
  if (currentPage === "body-map") {
    populateStudentCheckboxes();
  }

  // Update the visualization based on the current page
  updateAllVisualizations();
}
...
function initializeBodyMap() {
  ...
  torso.setAttribute("data-metrics", "HR,IBI");
  ...
  leftArm.setAttribute("data-metrics", "EDA,TEMP");
  ...
  rightArm.setAttribute("data-metrics", "EDA,TEMP");
  ...
  leftWrist.setAttribute("data-metrics", "EDA,TEMP");
  ...
  rightWrist.setAttribute("data-metrics", "EDA,TEMP");
  ...
  leftLeg.setAttribute("data-metrics", "TEMP");
  ...
  rightLeg.setAttribute("data-metrics", "TEMP");
  ...
  leftFoot.setAttribute("data-metrics", "TEMP");
  ...
  rightFoot.setAttribute("data-metrics", "TEMP");
...
function normalizeMetricValue(value, metric) {
  // These normalization factors are based on typical ranges for each metric
  switch (metric) {
    case "HR":
      // For heart rate: 40-180 BPM range mapped to 0-1
      return Math.max(0, Math.min(1, (value - 40) / 140));
    case "EDA":
      // For EDA: 0-25 μS range mapped to 0-1
      return Math.max(0, Math.min(1, value / 25));
    case "TEMP":
      // For temperature: 30-40 °C range mapped to 0-1
      return Math.max(0, Math.min(1, (value - 30) / 10));
    case "IBI":
      // For IBI: 0.3-2.0 seconds range mapped to 0-1
      return Math.max(0, Math.min(1, (value - 0.3) / 1.7));
    default:
      return value;
  }
}
...
function analyzeStressRecoveryPatterns(dataset, student, metric = "EDA") {
  ...
  if (metric === "EDA") {
    ...
    windowSize = 40; // Temperature changes slower
    thresholdPercent = 0.05; // 5% temperature change is significant
    absoluteThreshold = 0.5; // °C - minimum absolute change
  } else if (metric === "IBI") {
    windowSize = 20; // Inter-beat interval can change quickly
    thresholdPercent = 0.15; // 15% change in IBI
    absoluteThreshold = 0.05; // seconds - minimum absolute change
  }
...
function visualizeStressRecovery() {
  ...
  } else if (recoveryMetric === "TEMP") {
    metricExplanation =
      "Skin Temperature changes during stress as blood vessels constrict. Recovery time measures how quickly peripheral circulation and temperature regulation return to normal.";
  } else if (recoveryMetric === "IBI") {
    metricExplanation =
      "Inter-Beat Interval (IBI) reflects the time between heartbeats, which changes during stress. Recovery time shows how quickly heart rhythm regulation stabilizes.";
  }
  ...
  const allMetrics = ["EDA", "HR", "TEMP", "IBI"].filter(
    (m) => m !== recoveryMetric
  );
  ...
  if (recoveryMetric === "EDA") {
    ...
    explanationSection
      .append("p")
      .html(
        "<strong>Temperature Recovery:</strong> Skin temperature often decreases during stress due to peripheral vasoconstriction. The recovery pattern shows how quickly normal blood flow and temperature regulation return after a stressful event."
      );
  } else if (recoveryMetric === "IBI") {
    explanationSection
      .append("p")
      .html(
        "<strong>IBI Recovery:</strong> Inter-beat interval measures the time between heartbeats, which typically decreases during stress. IBI recovery patterns provide detailed insight into autonomic regulation of heart rhythm during and after stress."
      );
  }
...
if (currentPage === "landing" || currentPage === "") {
  ...
  const basePath = getBasePath();
  Promise.all([
    d3.json(`${basePath}json/Midterm_1.json`).catch((error) => {
      console.error("Error loading midterm1 data:", error);
      return null;
    }),
    d3.json(`${basePath}json/Midterm_2.json`).catch((error) => {
      console.error("Error loading midterm2 data:", error);
      return null;
    }),
    d3.json(`${basePath}json/Final.json`).catch((error) => {
  ...
  function getCurrentEmilyDataset() {
    switch (currentExam) {
      case "midterm1":
      case "Midterm_1":
        return landingPageMidterm1Data;
      case "midterm2":
      case "Midterm_2":
        return landingPageMidterm2Data;
      case "final":
      case "Final":
        return landingPageFinalData;
      default:
        console.warn("Unknown exam type:", currentExam);
        return null;
    }
  }
  ...
  function renderEmilyChart(metric, exam) {
    const dataset = getCurrentEmilyDataset(exam);

    if (!dataset) {
      console.error("No dataset available for the selected exam:", exam);
      return;
    }

    // Get data for the selected metric
    const data = getEmilyMetricData(metric);
    ...
    if (!data || data.length === 0) {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        if (metric === "HR") {
          value = 70 + 15 * Math.sin(i / 10) + Math.random() * 5;
        } else if (metric === "EDA") {
          value = 2 + 0.5 * Math.sin(i / 15) + Math.random() * 0.3;
        } else if (metric === "TEMP") {
          value = 32 + 0.5 * Math.sin(i / 20) + Math.random() * 0.2;
        } else if (metric === "IBI") {
          value = 0.8 - 0.1 * Math.sin(i / 10) + Math.random() * 0.05;
        } else {
    ...
    // Add Y axis with metric-specific label
    svg
      .append("g")
    ...
    // Add event markers
    const events = getExamEvents(exam);
    ...
    const eventGroups = svg
      ...
      .attr(
        "transform",
        (d) => `translate(${x(d.time)},${y(getEventValue(d.time, data))})`
      );
  ...
  // Helper to get the value at a specific time point
  function getEventValue(time, data) {
  ...
  // Helper to get event markers for each exam
  function getExamEvents(exam) {
    if (exam === "midterm1") {
      return [
        { time: 300, label: "First Question" },
        { time: 1800, label: "Time Warning" },
        { time: 3600, label: "Exam End" },
      ];
    } else if (exam === "midterm2") {
      return [
        { time: 600, label: "Difficult Section" },
        { time: 2400, label: "Time Warning" },
        { time: 4200, label: "Exam End" },
      ];
    } else if (exam === "final") {
      return [
        { time: 1800, label: "First Break" },
        { time: 5400, label: "Time Warning" },
        { time: 9000, label: "Exam End" },
      ];
    }
  ...
  // Helper to get metric axis label
  ...
  // Setup metrics grid click handlers
  function setupMetricsGrid() {
    const metricIcons = document.querySelectorAll(".metric-icon");
    metricIcons.forEach((icon) => {
      icon.addEventListener("click", function () {
        const metric = this.getAttribute("data-metric");
        const chartOverlay = document.querySelector(".chart-modal-overlay");
        const chartTitle = document.querySelector(".chart-title .metric-name");

        // Update chart title with selected metric
        chartTitle.textContent = getMetricFullName(metric);

        // Make overlay visible
        chartOverlay.classList.add("visible");

        // Render the chart with selected metric
        renderEmilyChart(metric, "midterm1");

        // Setup exam button listeners
        const examButtons = document.querySelectorAll(".exam-button");
        examButtons.forEach((btn) => {
          btn.addEventListener("click", function () {
            ...
            this.classList.add("active");
            // Render chart with selected exam
            renderEmilyChart(metric, this.getAttribute("data-exam"));
          });
        });
      });
    });
    ...
    closeButton.addEventListener("click", function () {
      document
        .querySelector(".chart-modal-overlay")
    ...
    landingPageMidterm1Data = midterm1Data;
    landingPageMidterm2Data = midterm2Data;
    landingPageFinalData = finalData;

    // Try to load the full datasets
    d3.json(`${basePath}json/Midterm_1.json`)
      .then((data) => {
        console.log("Successfully loaded midterm1 data for Emily's metrics");
        landingPageMidterm1Data = data;
      })
      .catch((error) => {
        console.warn("Using sample data for Emily's Midterm 1 metrics:", error);
      });

    d3.json(`${basePath}json/Midterm_2.json`)
      .then((data) => {
        console.log("Successfully loaded midterm2 data for Emily's metrics");
        landingPageMidterm2Data = data;
      })
      .catch((error) => {
        console.warn("Using sample data for Emily's Midterm 2 metrics:", error);
      });

    d3.json(`${basePath}json/Final.json`)
      .then((data) => {
        console.log("Successfully loaded final data for Emily's metrics");
        landingPageFinalData = data;
      })
      .catch((error) => {
        console.warn("Using sample data for Emily's Final metrics:", error);
      });
  }
}
